...
int main(int argc, char** argv)
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glClear(GL_DEPTH_BUFFER_BIT);
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, turkeyVuTexture);
		glEnable(GL_CULL_FACE);
		glCullFace(GL_FRONT);
		...
		glEnable(GL_CULL_FACE);
		glCullFace(GL_FRONT);
		renderCloud(shadowMappingDepthShader);
		glCullFace(GL_BACK);
		glBindFramebuffer(GL_FRAMEBUFFER, 0);
		...
		glClear(GL_DEPTH_BUFFER_BIT);
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, standerTexture);
		glEnable(GL_CULL_FACE);
		glCullFace(GL_FRONT);
		renderStander(shadowMappingDepthShader);
		glCullFace(GL_BACK);
		glBindFramebuffer(GL_FRAMEBUFFER, 0);
		...
		glEnable(GL_CULL_FACE);
		glCullFace(GL_FRONT);
		renderPiloni(shadowMappingDepthShader);
		glCullFace(GL_BACK);
		glBindFramebuffer(GL_FRAMEBUFFER, 0);
		...
		glBindTexture(GL_TEXTURE_2D, depthMap);
		glDisable(GL_CULL_FACE);
		renderGoose(shadowMappingShader);
		...
		glBindTexture(GL_TEXTURE_2D, depthMap);
		glDisable(GL_CULL_FACE);
		renderCloud(shadowMappingShader);
		...
		glBindTexture(GL_TEXTURE_2D, depthMap);
		glDisable(GL_CULL_FACE);
		renderStander(shadowMappingShader);
...
void renderCloud(const Shader& shader)
{
	glm::mat4 model;
	model = glm::mat4();
	model = glm::translate(model, glm::vec3(0.0f, 10.7f, -17.7f));
	...
	//glm::mat4 model;
	model = glm::mat4();
	model = glm::translate(model, glm::vec3(-7.0f, 10.2f, -17.7f));
	...
	//glm::mat4 model;
	model = glm::mat4();
	model = glm::translate(model, glm::vec3(6.0f, 10.7f, -17.7f));
...
void renderParrot(const Shader& shader)
{
	...
	glm::mat4 model;
	model = glm::mat4();
	model = glm::translate(model, glm::vec3(10.4f, -0.5f, -14.6f));
	model = glm::scale(model, glm::vec3(0.85f));

	//model = glm::translate(model, glm::vec3(-2.5f, -0.7f, -13.4f));
...
void renderPiloni(const Shader& shader)
{
	glm::mat4 model;
	model = glm::mat4();
	model = glm::translate(model, glm::vec3(0.f, -0.5f, 10.0f));
...
void renderTree(const Shader& shader)
{
	...
	glm::mat4 model;
	model = glm::mat4();
	model = glm::translate(model, glm::vec3(5.0f, -2.5f, -20.0f));
...
void renderOwl(const Shader& shader)
{
	...
	glm::mat4 model;
	model = glm::mat4();
	model = glm::translate(model, glm::vec3(9.5f, 2.4f, -14.7f));
	model = glm::scale(model, glm::vec3(1.f));
	shader.SetMat4("model", model);
	renderOwl();
...
void renderWoodpecker(const Shader& shader)
{
	glm::mat4 model;
	model = glm::mat4();
	model = glm::translate(model, glm::vec3(-8.24f, 2.0f, -16.2f));
	model = glm::scale(model, glm::vec3(1.8f));
...
void renderGoose(const Shader& shader)
{
	glm::mat4 model;
	model = glm::mat4();
	model = glm::translate(model, glm::vec3(-2.5f, -0.5f, -17.0f));
...
void renderPeacock(const Shader& shader)
{
	...
	glm::mat4 model;
	model = glm::mat4();
	model = glm::translate(model, glm::vec3(3.4f, 3.4f, -15.6f));
	model = glm::scale(model, glm::vec3(2.f));
...
void renderDino(const Shader& shader)
{
	...
	model = glm::mat4();
	model = glm::translate(model, glm::vec3(0.0f, 0.f, 12.5f));
	model = glm::scale(model, glm::vec3(0.75f));
	shader.SetMat4("model", model);
	renderTrexBottom();
	...
	model = glm::mat4();
	model = glm::translate(model, glm::vec3(-5.0f, 0.f, 12.5f));
	model = glm::scale(model, glm::vec3(0.75f));
	shader.SetMat4("model", model);
	renderTrexBottom();
...
float verticesCloud[82000];
unsigned int indicesCloud[72000];
...
void renderCloud()
{
	...
	if (cloudVAO == 0)
	{
		...
		for (int j = 0; j < verticess.size(); j++)
		{
			verticesCloud[j] = verticess.at(j);
		}
		...
		glBufferData(GL_ARRAY_BUFFER, sizeof(verticesCloud), verticesCloud, GL_DYNAMIC_DRAW);

		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, cloudEBO);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indicesCloud), &indicesCloud, GL_DYNAMIC_DRAW);
		// link vertex attributes
...
float verticesParrot[82000];
unsigned int indicesParrot[72000];
...
void renderParrot()
{
	...
	if (parrotVAO == 0)
	{
		...
		for (int j = 0; j < verticess.size(); j++)
		{
			verticesParrot[j] = verticess.at(j);
		}
		...
		glBufferData(GL_ARRAY_BUFFER, sizeof(verticesParrot), verticesParrot, GL_DYNAMIC_DRAW);
...
float verticesStander[82000];
unsigned int indicesStander[72000];

GLuint StanderVAO, StanderVBO, StanderEBO;
...
void renderStander()
{
	...
	if (StanderVAO == 0)
	{
		...
		for (int j = 0; j < verticess.size(); j++)
		{
			verticesStander[j] = verticess.at(j);
		}
		...
		for (int j = 0; j < curMesh.Indices.size(); j++)
		{
			indicesStander[j] = indicess.at(j);
		}

		glGenVertexArrays(1, &StanderVAO);
		glGenBuffers(1, &StanderVBO);
		glGenBuffers(1, &StanderEBO);
		// fill buffer
		glBindBuffer(GL_ARRAY_BUFFER, StanderVBO);
		glBufferData(GL_ARRAY_BUFFER, sizeof(verticesStander), verticesStander, GL_DYNAMIC_DRAW);

		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, StanderEBO);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indicesStander), &indicesStander, GL_DYNAMIC_DRAW);
		// link vertex attributes
		glBindVertexArray(StanderVAO);
		glEnableVertexAttribArray(0);
...
unsigned int indices99[72000];
objl::Vertex ver7[82000];
GLuint pilon1VAO, pilon1VBO, pilon1EBO;
void renderPilon1()
{
	// initialize (if necessary)
	if (pilon1VAO == 0)
	{
		...
		for (int j = 0; j < curMesh.Indices.size(); j++)
		{
			indices99[j] = indicess.at(j);
		}

		glGenVertexArrays(1, &pilon1VAO);
		glGenBuffers(1, &pilon1VBO);
		glGenBuffers(1, &pilon1EBO);
		// fill buffer
		glBindBuffer(GL_ARRAY_BUFFER, pilon1VBO);
		glBufferData(GL_ARRAY_BUFFER, sizeof(ver7), ver7, GL_DYNAMIC_DRAW);

		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, pilon1EBO);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices99), &indices99, GL_DYNAMIC_DRAW);
		// link vertex attributes
		glBindVertexArray(pilon1VAO);
		glEnableVertexAttribArray(0);
...
GLuint pilon2VAO, pilon2VBO, pilon2EBO;
void renderPilon2()
{
	// initialize (if necessary)
	if (pilon2VAO == 0)
	{
		...
		glGenVertexArrays(1, &pilon2VAO);
		glGenBuffers(1, &pilon2VBO);
		glGenBuffers(1, &pilon2EBO);
		// fill buffer
		glBindBuffer(GL_ARRAY_BUFFER, pilon2VBO);
		glBufferData(GL_ARRAY_BUFFER, sizeof(ver7), ver7, GL_DYNAMIC_DRAW);

		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, pilon2EBO);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices99), &indices99, GL_DYNAMIC_DRAW);
		// link vertex attributes
		glBindVertexArray(pilon2VAO);
		glEnableVertexAttribArray(0);
...
void renderWoodpecker()
{
	...
	if (woodpeckerVAO == 0)
	{
		...
		Loader.LoadFile("woodpeacker.obj");
		objl::Mesh curMesh = Loader.LoadedMeshes[0];
		int size = curMesh.Vertices.size();
		...
		glBufferData(GL_ARRAY_BUFFER, sizeof(vertices50), vertices50, GL_DYNAMIC_DRAW);

		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, woodpeckerEBO);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices50), &indices50, GL_DYNAMIC_DRAW);
		// link vertex attributes
...
float vertices51[82000];
unsigned int indices51[72000];
GLuint gooseVAO, gooseVBO, gooseEBO;
...
void renderGoose()
{
	...
	if (gooseVAO == 0)
	{
		...
		for (int j = 0; j < curMesh.Indices.size(); j++)
		{
			indices51[j] = indicess.at(j);
		}

		glGenVertexArrays(1, &gooseVAO);
		glGenBuffers(1, &gooseVBO);
		glGenBuffers(1, &gooseEBO);
		// fill buffer
		glBindBuffer(GL_ARRAY_BUFFER, gooseVBO);
		glBufferData(GL_ARRAY_BUFFER, sizeof(vertices51), vertices51, GL_DYNAMIC_DRAW);

		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, gooseEBO);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices51), &indices51, GL_DYNAMIC_DRAW);
		// link vertex attributes
